...
unsigned char Button::process() {
    ...
    /*
    if(reading==0) {    // if button is either bouncing or stabilised in LOW
        if(prev_read==1) { // falling edge detected
            last_debounce_time = millis();  // reset debounce timer
        }
        else if(button_state==1) {  // not a falling edge since
            if((millis() - last_debounce_time) > 50) {  // 50ms debounce time
                button_state = 0;
                prev_read = reading;
        ...
        button_state = 1;   // button released
    }
    prev_read = reading;   // update state for next loop
